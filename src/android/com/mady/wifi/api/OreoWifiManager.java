...
import com.mady.wifi.datatransfer.WifiSocket;
...
public class OreoWifiManager {
    ...
    public boolean configureHotspot(String name, String password) {
        ...
        apConfig.preSharedKey = randomUUID.substring(0, 8) + randomUUID.substring(9, 13);
        try {
            Method method = mWifiManager.getClass().getMethod("setWifiApConfiguration", WifiConfiguration.class);
            method.setAccessible(true);
            boolean status = (boolean) method.invoke(mWifiManager, apConfig);
            Log.d(TAG, "setWifiApConfiguration - success? " + status);
            return status;
        } catch (Exception e) {
            Log.e(TAG, "Error in configureHotspot", e);
            e.printStackTrace();
            return false;
        }
    }
